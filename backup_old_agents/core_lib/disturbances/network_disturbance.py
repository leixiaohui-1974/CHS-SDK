...
class NetworkDelayDisturbance:
    ...
    def activate(self, start_time: float, end_time: float):
        """激活扰动"""
        ...
        logger.info(f"网络延迟扰动 {self.disturbance_id} 已激活，结束时间: {end_time}s")
...
class PacketLossDisturbance:
    ...
    def activate(self, start_time: float, end_time: float):
        ...
        logger.info(f"数据包丢失扰动 {self.disturbance_id} 已激活，结束时间: {end_time}s")
