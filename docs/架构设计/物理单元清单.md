...
| 物理单元 | 功能与物理特性描述 | 对应的智能体 | 源码路径 |
| :--- | :--- | :--- | :--- |
| **通用控制器** | **本地控制通用基类**，封装了控制器，并通过消息总线处理观测和动作。是实现大多数本地控制任务的核心。 | `LocalControlAgent` | `core_lib/local_agents/control/local_control_agent.py` |
| 泵机组 | 控制一个**泵站**（PumpStation），根据流量需求决定开启或关闭站内**多个离散水泵**。(*注意: 该代理直接继承自 Agent，而非 LocalControlAgent*) | `PumpControlAgent` | `core_lib/local_agents/control/pump_control_agent.py` |
| 单个阀门（控制） | 控制单个阀门。它本身是 `LocalControlAgent` 的一个轻量级实现，需在初始化时配置具体的控制器（如PID）。 | `ValveControlAgent` | `core_lib/local_agents/control/valve_control_agent.py` |
| 单个闸门（控制） | 控制单个闸门，与 `ValveControlAgent` 类似，是 `LocalControlAgent` 的轻量级实现。 | `GateControlAgent` | `core_lib/local_agents/control/gate_control_agent.py` |
| 泵站（控制） | 泵站控制的**站级代理**，负责站级总调度（如决定总流量），并将指令下发给机组级代理（如 `PumpControlAgent`）。 | `PumpStationControlAgent` | `core_lib/local_agents/control/pump_station_control_agent.py` |
| 阀门站（控制） | 阀门站控制的**站级代理**，负责实现站级流量或水位目标，并将计算出的开度指令下发给站内每个阀门。 | `ValveStationControlAgent` | `core_lib/local_agents/control/valve_station_control_agent.py` |
| 水电站（控制） | 水电站控制，负责制定站级策略，并向下游发布针对单个水轮机或水闸的控制指令。 | `HydropowerStationAgent` | `core_lib/local_agents/control/hydropower_station_agent.py` |
| 水电站（控制） | 水电站控制，同上。 | `HydropowerStationControlAgent` | `core_lib/local_agents/control/hydropower_station_control_agent.py` |
| 压力控制器 | 压力控制，一个 `LocalControlAgent` 的应用实例，专用于需要维持特定压力的控制场景。 | `PressureControlAgent` | `core_lib/local_agents/control/pressure_control_agent.py` |
...
## 5. 外部影响与边界条件 (External Influences & Boundary Conditions)
...
本文档为CHS-SDK水系统仿真平台提供了完整的物理单元参考。通过清晰的分类和详细的描述，开发者可以：

1. **快速理解**：了解每个物理单元的功能和特性
2. **正确建模**：选择合适的物理模型和智能体
3. **系统设计**：构建符合实际工程需求的水利系统
4. **扩展开发**：基于现有框架添加新的物理单元类型

建议开发者在进行系统建模时，严格按照本文档的分类和命名规范，确保系统的一致性和可维护性。
