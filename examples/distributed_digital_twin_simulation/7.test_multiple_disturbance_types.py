...
def inflow_disturbance(harness):
    """测试入流扰动"""
    logger.info("\n=== 测试入流扰动 ===")
...
def sensor_noise_disturbance(harness):
    """测试传感器噪声扰动"""
    logger.info("\n=== 测试传感器噪声扰动 ===")
...
def multiple_disturbances():
    ...
    # 添加不同类型的扰动
    inflow_id = inflow_disturbance(harness)
    sensor_id = sensor_noise_disturbance(harness)
