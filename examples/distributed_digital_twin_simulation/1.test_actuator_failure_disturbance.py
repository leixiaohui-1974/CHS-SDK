...
def actuator_failure_disturbances():
    """测试执行器故障扰动"""
    ...
    # 添加不同类型的执行器故障扰动
    delay_id = delay_failure(harness)
    
    if downstream_reservoir:
        partial_id = partial_failure(harness)
    else:
        partial_id = None
        logger.info("跳过部分故障测试（缺少下游水库）")
    
    complete_id = complete_failure(harness)
