...
def main():
    """
    Main function to run the gate identification scenario.
    """
    print("Starting gate discharge coefficient identification example...")
    ...
    # --- 5. Define and Add a Custom Agent for Gate Control Events ---
    class GateControlEventAgent(Agent):
        ...
        def run(self, current_time: float):
            if current_time == 500:
                print(f"  [{current_time}s] Event: Setting gate opening to 0.8")
                self.gate.target_opening = 0.8
            elif current_time == 10000:
                print(f"  [{current_time}s] Event: Setting gate opening to 0.3")
    ...
    # --- 6. Get Final Parameters and Show Results ---
    final_twin_coeff = twin_gate.get_parameters()['discharge_coefficient']

    print("\nIdentification process complete. Comparing results...")
    print("-" * 50)
    print("Discharge Coefficient (C) Identification")
    print("-" * 50)
    print(f"True Value:      {true_coeff:.4f}")
    print(f"Initial Guess:   {initial_twin_coeff:.4f}")
    print(f"Final Identified:{final_twin_coeff:.4f}")
    print("-" * 50)
