...
class ParameterIdentificationAgent(Agent):
    ...
    def __init__(self, agent_id: str, target_model: Identifiable,
        ...
        Args:
            ...
            **kwargs: Agent-specific configuration containing:
                - identification_interval (int): 在运行一次辨识前需要收集的数据点数量。
                - identification_data_map (dict): 将模型的 identify_parameters 方法所需的
                                                  键映射到它们对应的主题和数据键。
        """
        ...
        for model_key, data_config in self.data_map.items():
            ...
            data_key = data_config.get('key', 'value') # Default to 'value'
    ...
    def handle_data_message(self, message: Message, model_key: str, data_key: str):
        ...
        if isinstance(value, (int, float)):
            ...
            # Only increment the counter for the first data stream to ensure sync
